        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function(key, value) {
                    ...
                    var namaEP = data[key]['ep'].replace(/[!"#$%&'()*+,.\/:;<=>?@[\\\]^`{|}~]/g, "\\\\$&")
                    $("#"+namaEP).append(`<iframe src="{{ url('files/` + namafile + `') }}" align="top" height="620" width="100%" frameborder="0" scrolling="auto"></iframe>`)
                })
            },
            error: function(data) {
                console.log('error')
        ...
        $.ajax({
            ...
            success: function(data) {
                $.each(data, function(key, value) {
                    var nilai;
                    var ep = data[key]['ep']
                    var bab = data[key]['bab']
                    var nameidEP = data[key]['ep'].replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '-')
                    ...
                    <div class="modal fade" id="modalDocument_` + key + `" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            ...
                            <div class="modal-body" id="`+nameidEP+`" >
